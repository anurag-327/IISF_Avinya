...
const ResultCard = ({item}) => {
  ...
  return (
    <div onClick={() => setOpen(!isOpen)} className="w-[60%] max-w-[600px] relative sm:w-full flex flex-col bg-gradient-to-r from-blue-600 to-green-700 p-2 rounded-md">
      <div className={isOpen? "rounded-t-md row1 flex justify-between w-full p-2 ": "rounded-md row1 flex justify-between w-full p-2 "}>
        <div className=" flex gap-5">
          <h2>{item.rank}</h2>
          {
             item.rank === 1 || item.rank === 2 || item.rank === 3 ? (<Crown size={25} color="#ffffff" weight="fill" />) : (<SmileyBlank size={25} color="#17141a" weight="fill" />)
          }
      ...
      </div>
      {
         ...
         {
             item.members.map((mem,index) => <span className='sm:mx-5'>{mem}, </span>)
         }
        </div>
      }
    </div>
